...
def update_status(equipment_id: int, status: str, db: Session = Depends(database.get_db)):
    return crud.update_status(db, equipment_id, status)

@router.put("/{equipment_id}/move")
def move_equipment(equipment_id: int, to_room: str, db: Session = Depends(database.get_db)):
    return crud.move_equipment(db, equipment_id, to_room)
